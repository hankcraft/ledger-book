  ...
  },
  {
    ...
    attentionItems: [
      ...
      },
      {
        id: "vs-benchmark",
        label: "相對大盤表現",
        detail: "大盤跌 1.8%，你跌 2.6%。差異主要來自集中持股。",
        severity: "INFO",
      },
    ],
  ...
  const messages: V1StreamMessage[] = [
    { id: "turn-1", role: "agent", text: "我先幫你分清楚：今天真的發生了多大的變化？" },
    {
      ...
      card: {
        ...
        marketChange: "-1.5%",
        breakdown: [
          { label: "台積電", value: "-3.2%", note: "佔持倉 40%" },
          { label: "聯發科", value: "-4.1%", note: "佔持倉 28%" },
        ],
      },
    },
    {
...
export function createV1Routes(store = createV1Store()) {
  return (
    new Elysia({ prefix: "/api/v1" })
      ...
      // ─── Home ────────────────────────────────────────────
      .get("/home/daily-performance", () => {
        // Compute weighted daily return from holdings
        const totalWeight = store.context.holdings.reduce((sum, h) => sum + h.weight, 0);
        const portfolioReturn =
          totalWeight > 0
            ? store.context.holdings.reduce(
                (sum, h) => sum + (h.plPercent * h.weight) / totalWeight,
                0,
              ) * 0.1
            : 0;
        const benchmarkReturn = 0.8; // Simulated 0050 daily return
      ...
      .get("/home/scenario", async () => {
        ...
        try {
          ...
          console.log(
            "[v1] Agent unavailable for scenario, using seed insight:",
            err instanceof Error ? err.message : err,
          );
        }
            ...
            messages = [
              {
                ...
                role: "agent",
                text: `你選擇先釐清「${conv.selectedOption ?? body.text}」。我會把這個問題和你的持倉、過去記憶一起整理。`,
              },
            ];
          }
